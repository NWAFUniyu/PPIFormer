...
class roc_callback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        self.model.save_weights("./model/pioneer_onehot/rcnn/%s/%sModel%d.h5" % (self.name, self.name, epoch))
...
def build_model():
    seq_input1 = Input(shape=(seq_size, 21), name='seq1')
    seq_input2 = Input(shape=(seq_size, 21), name='seq2')
...
for name in Virus_Species:
    os.makedirs('./model/pioneer_onehot/rcnn/' + name, exist_ok=True)
