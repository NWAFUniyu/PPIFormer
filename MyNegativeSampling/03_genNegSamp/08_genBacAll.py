...
bac_res_train = 'dataset/train/bacteria/all/'
bac_res_test = 'dataset/test/bacteria/all/'
...
interactions_data = read_csv('data/positive/bacteria/all/All.csv')
possible_interactions_data = read_csv('data/possible/BacPossibleTable.csv')

# 从CSV文件读取Species测试数据，仅保留前两列
Species_testPositivedata = read_csv('dataset/test/bacteria/species/TestPositiveAll.csv')
Species_testNegativedata = read_csv('dataset/test/bacteria/species/TestNegativeAll.csv')
...
# 合并数据集并保存为CSV文件
train_file_path = os.path.join(bac_res_train,'All_train.csv')
test_file_path = os.path.join(bac_res_test, 'All_test.csv')
