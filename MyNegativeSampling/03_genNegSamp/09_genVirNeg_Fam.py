...
Virus_family = ["Coronaviridae", "Filoviridae", "Flaviviridae", "Herpesviridae", "Orthomyxoviridae", "Paramyxoviridae",
...
for name in Virus_family:
    print(name)
    vir_res_train = 'dataset/train/virus/family/' + name + '/'
    vir_res_test = 'dataset/test/virus/family/' + name + '/'
    create_directory(vir_res_train)
    create_directory(vir_res_test)

    interactions_data = read_csv('data/positive/virus/family/' + name + '/' + name + '.csv')
    possible_interactions_data = read_csv('data/possible/VirPossibleTable.csv')

    # 从CSV文件读取Species测试数据，仅保留前两列
    Species_testPositivedata = read_csv('data/TestPositiveSpe.csv')
    Species_testNegativedata = read_csv('data/TestNegativeSpe.csv')
