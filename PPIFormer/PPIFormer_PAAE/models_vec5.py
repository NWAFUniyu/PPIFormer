...
def get_model(max_len_pr1, max_len_pr2):
    #input
    pr1 = Input(shape=(max_len_pr1,13,))#word2vec 维度可以调整 200
    pr2 = Input(shape=(max_len_pr2,13,))

    #pr1
    pr1_conv_layer = Conv1D(filters=64,#
                                    kernel_size=40,#
                                    padding="valid",
                                    activation='relu'
                                    )(pr1)
    #再加一层CNN
    pr1_max_pool_layer = MaxPooling1D(pool_size=5, strides=5)(pr1_conv_layer)#3
